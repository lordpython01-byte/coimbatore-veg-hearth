...
const KolamPattern = () => <svg className="absolute inset-0 w-full h-full opacity-20 pointer-events-none" xmlns="http://www.w3.org/2000/svg">
    <defs>
...
const Menu = () => {
  ...
  const fetchMenuData = async () => {
    try {
      ...
      } = await supabase.from("menu_categories").select("*").eq("is_active", true).order("display_order");
      ...
      const {
        ...
        `).eq("is_available", true).order("display_order").limit(9);
  ...
  const filteredItems = selectedCategory === "All" ? menuItems : menuItems.filter(item => item.category_name === selectedCategory);
  useEffect(() => {
    setVisibleCards([]);
  }, [selectedCategory]);
  useEffect(() => {
  ...
  return <section id="menu" className="py-20 bg-background relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          ...
          <div className="flex justify-center items-center gap-3">
            ...
            <div className="flex gap-1">
              {[...Array(5)].map((_, i) => <div key={i} className="w-2 h-2 rotate-45 bg-primary opacity-60"></div>)}
            </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {allCategories.map(category => <Button key={category} variant={selectedCategory === category ? "default" : "outline"} onClick={() => setSelectedCategory(category)} className={`rounded-full px-6 py-2 transition-all duration-300 ${selectedCategory === category ? "shadow-lg scale-105" : "hover:scale-105"}`}>
              {category}
            </Button>)}
        </div>
        ...
        {loading ? <div className="text-center py-20">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 max-w-6xl mx-auto relative">
              {filteredItems.map((item, index) => <div key={item.id} ref={el => cardRefs.current[index] = el} className={`transition-all duration-500 ${visibleCards[index] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'} ${index === 8 ? 'relative overflow-hidden' : ''}`} style={{
                    ...
                    <CardContent className="p-0 flex flex-col h-full">
                      ...
                      <div className="p-3 md:p-4 flex flex-col flex-grow">
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-border">
                          ...
                          <div className="flex gap-1">
                            {[...Array(3)].map((_, i) => <div key={i} className="w-1 h-1 rounded-full bg-accent"></div>)}
                          </div>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                  {index === 8 && <div className="absolute inset-0 bg-gradient-to-b from-transparent via-background/50 to-background pointer-events-none z-10"></div>}
                </div>)}
            </div>
        ...
        <div className="text-center mt-16">
          <Button size="lg" onClick={() => navigate('/full-menu')} className="group relative overflow-hidden bg-gradient-to-r from-primary to-accent hover:from-primary-glow hover:to-accent text-primary-foreground font-bold text-lg px-8 py-6 rounded-full shadow-lg hover:shadow-2xl transition-all duration-300">
